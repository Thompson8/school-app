...
export class StudentListComponent implements OnInit {
  ...
  columns = [{ id: '1', name: 'First name', property: 'firstName', isAction: null },
  { id: '2', name: 'Last name', property: 'lastName', isAction: null },
  { id: '3', name: null, property: '', isAction: true }];

  dataSource: NbTreeGridDataSource<StudentListItemDto>;
  ...
  constructor(private studentService: StudentService, private dataSourceBuilder: NbTreeGridDataSourceBuilder<StudentListItemDto>, private dialogService: NbDialogService) {
  ...
  loadData() {
    ...
    this.loading = true;
    this.studentService.getStudents(this.filterValue, this.pageNumber, this.pageSize, sorting, this.sortDirection).subscribe(e => this.updateData(e));
  }

  updateData(list: StudentListDto) {
  ...
  openCreateDialog() {
    let dialogRef = this.dialogService.open(StudentDialogComponent, { context: { mode: 'CREATE' } });
    dialogRef.onClose.subscribe(e => this.refresh());
  }

  openViewDialog(id: number) {
    let dialogRef = this.dialogService.open(StudentDialogComponent, { context: { studentId: id, mode: 'VIEW' } });
    dialogRef.onClose.subscribe(e => this.refresh());
  }

  openEditDialog(id: number) {
    let dialogRef = this.dialogService.open(StudentDialogComponent, { context: { studentId: id, mode: 'MODIFY' } });
    dialogRef.onClose.subscribe(e => this.refresh());
  }

  openDeleteDialog(id: number) {
    let dialogRef = this.dialogService.open(DeleteDialogComponent, { context: { action: this.studentService.deleteStudent(id) } });
    dialogRef.onClose.subscribe(e => this.refresh());
  }
